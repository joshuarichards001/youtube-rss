...
export const syncSubscriptions = async (
  ...
  try {
    ...
    do {
      ...
      if (!response.ok) {
        ...
        res
          ...
          .json({ error: "Failed to fetch subscriptions from YouTube" });
        return;
      }
    ...
    console.log(
      `[Sync] Total subscriptions fetched from YouTube: ${allItems.length}`,
    );
    const subscriptions: Subscription[] = allItems.map(
      mapYouTubeSubscriptionToSubscription,
    );
    ...
    // 3. Upsert Channels
    console.log(
      `[Sync] Preparing to upsert ${channelsToUpsert.length} channels`,
    );
    ...
    console.log(
      `[Sync] Found ${existingSubs?.length || 0} existing subscriptions`,
    );
    ...
    const toAdd = subscriptions.filter(
      (s) => !existingChannelIds.has(s.channelId),
    );
    const toRemove = [...existingChannelIds].filter(
      (id) => !newChannelIds.has(id),
    );

    console.log(
      `[Sync] Calculations: ${toAdd.length} to add, ${toRemove.length} to remove`,
    );
    ...
    console.log(
      `[Sync] Database sync complete. Queuing ${subscriptionsToRssSync.length} channels for RSS fetch...`,
    );

    // Fire-and-forget the worker
    if (subscriptionsToRssSync.length > 0) {
      processSubscriptions(subscriptionsToRssSync, user.id).catch(
        (err: unknown) => {
